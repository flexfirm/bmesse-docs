...
ユーザーには「オンライン」状態と、「オフライン」状態があります。  
これは、ユーザーの状態（例えばサービスの「ログイン」と「ログオフ」）と、  
...
オペレーターが、複数のアプリユーザーと同時チャットできる人数を設定します。  
```
...
例えば、3人のアプリユーザーと会話をしたオペレーターの場合、3人分のuserInfoを取得するイベントが走ります。  
...
付加情報の設定の仕方について、詳しくは[iOS_SDK導入ガイド](./iOS_SDK導入ガイド.md)をご参照ください。  
